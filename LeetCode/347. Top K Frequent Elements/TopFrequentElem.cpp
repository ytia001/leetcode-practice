...
class Solution {
public:
     vector<int> topKFrequentTest2(vector<int>& nums, int k) {
      sort(nums.begin(),nums.end());
      vector<int> res;
      priority_queue<pair<int,int>> freqQ;
      ...
      for(auto pr: freqMap){
        freqQ.push(make_pair(pr.second,pr.first));
      }

      for(int i = 0 ; i < k ; i++){
        res.push_back(freqQ.top().second);
        freqQ.pop();
      }
    ...
    vector<int> topKFrequent(vector<int>& nums, int k) {
      ...
      for(int i = 0 ; i< nums.size() ; i++){
        currentNum = nums[i];
        currentFreq++;
        if(i==nums.size()-1 || nums[i+1]!=currentNum){
          //cout<<"currentFreq: "<<currentFreq<<"\tcurrentNum: "<<currentNum<<endl;
          freqQ.push({currentFreq,currentNum});
          currentFreq=0;  
        }
      }
    ...
    vector<int> topKFrequentTest(vector<int>& nums, int k) {
      vector<int> res;
      unordered_map<int,int> freqMap;
      ...
      vector<pair<int,int>> freqVect(freqMap.begin(),freqMap.end());
      sort(freqVect.begin(),freqVect.end(),comparator);

      // for(auto pr: freqVect){
      //       cout<<"Num: "<<pr.first<<"count: "<<pr.second<<endl;
      //  }
      res.clear();
      for(int i = 0 ; i < k ; i++){
        res.push_back(freqVect[i].first);
      }
